    ...
    if barLabel:
        try:
            for j,i in enumerate(rects.values()):
                ax.bar_label(i, padding=3, labels=[barLabel[0][:].format(ldata[j][ldata[0].index[r]], barLabel[j+1][r]) for r in range(len(ldata[0]))], rotation = labRot)
        except:
            for j,i in enumerate(rects.values()):
                ax.bar_label(i, padding=3, labels=['{}\n{:.2f}%'.format(ldata[j][ldata[0].index[r]], barLabel[j][r]) for r in range(len(ldata[0]))], rotation = labRot)
    else:
