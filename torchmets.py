...
#-------------- Curve Metrics Calculated by Sklearn ---------------------#

def compute_full_metrics(y_true, y_prob):
    y_true = y_true.astype(np.int32)

    uap = skm.roc_auc_score(y_true.reshape(-1), y_prob.reshape(-1))

    auc = [skm.roc_auc_score(y_t, y_p)
    ...
    iap = np.average([skm.roc_auc_score(y_t, y_p)
                      for y_t, y_p in zip(y_true, y_prob) if not (y_t == np.zeros(y_t.shape)).all()])
    y_pred = np.round(y_prob)
    f1 = skm.f1_score(y_true, y_pred, average='micro')

    prec = skm.precision_score(y_true, y_pred, average='micro')

    rec = skm.recall_score(y_true, y_pred, average='micro')

    aucs = [skm.roc_auc_score(t, p) for t, p in zip(y_true.T, y_prob.T)]

    return [uap, map, wap, iap, f1, prec, rec] + aucs
